...
try:
    ...
    if not bulls_row.empty:
        ...
        print("No Bulls data found in stats endpoints")
except Exception as e:
    print(f"Stats error: {e}")

# 2. LIVE SCOREBOARD - All Games Today (Detailed Print)
try:
    ...
    for g in games:
        ...
        # Print each game in a clean format
        score_str = f"{away_score} - {home_score}" if away_score > 0 else "N/A"
        live_tag = " LIVE " if is_live else ""
        complete_tag = " FINAL " if is_completed else ""
        print(f"{away_name} @ {home_name}  |  {score_str}  |  Q{quarter}  {clock}  |  {status_text}{live_tag}{complete_tag}")
